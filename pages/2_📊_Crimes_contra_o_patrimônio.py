...
# 4.4.7 Roubo em coletivo
roubo_col_ano = px.line(df_hist_anual.groupby('ano')[['roubo_em_coletivo']].apply(lambda x: x.sum()/1000).round(2).reset_index(),
                        x='ano', y='roubo_em_coletivo', markers=True, text='roubo_em_coletivo', line_shape="spline", template="plotly_dark",
                        title="Roubo em coletivo", height=525, width=850,
                        color_discrete_sequence=px.colors.sequential.Blackbody_r,
                        labels=dict(ano="Ano", roubo_em_coletivo="Roubos (k)")
                        )
roubo_col_ano.update_layout(xaxis=dict(linecolor='rgba(0,0,0,1)', tickmode='array',
                            tickvals=df_hist_anual['ano'], ticktext=df_hist_anual['ano']))
roubo_col_ano.add_annotation(
    x=2020, y=9.23, font=dict(color="red", size=12), text="Covid-19", showarrow=True, arrowhead=2, ax=-50, ay=0)
roubo_col_ano.update_layout(showlegend=False)
roubo_col_ano.update_traces(line_width=2, textposition='top center')

# 4.4.8 Estelionato
estelionato_ano = px.line(df_hist_anual.groupby('ano')[['estelionato']].apply(lambda x: x.sum()/1000).round(1).reset_index(),
                          x='ano', y='estelionato', markers=True, text='estelionato', line_shape="spline", template="plotly_dark",
                          title="Estelionato", height=525, width=850,
                          color_discrete_sequence=px.colors.sequential.Blackbody_r,
                          labels=dict(ano="Ano", estelionato="Casos (k)")
                          )
estelionato_ano.update_xaxes(type="category", title=None)
estelionato_ano.update_layout(showlegend=False)
estelionato_ano.update_traces(line_width=2, textposition='top center')

# 4.4.9 Extorção
extorcao_ano = px.line(df_hist_anual.groupby(['ano'])['extorsao'].sum().reset_index(),
                       x='ano', y=['extorsao'],
                       markers=True, text='value', line_shape="spline", template="plotly_dark",
                       title="Extorção",
                       height=525, width=850,  # altura #largura
...
extorcao_ano.update_xaxes(type="category", title=None)
extorcao_ano.update_layout(showlegend=False)
extorcao_ano.update_traces(line_width=2, textposition='top center')
